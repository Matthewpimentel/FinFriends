...
const Feed = () => {
  ...
  const getFeed = async () => {
    try {
      let response;
      if (user?.email) {
        response = await axios.get("/api/get-feed", {
          params: {
            email: user.email
          }
        });
      } else {
        response = await axios.get("/api/get-no-user-feed")
      }
            ...
            feed.map((post) => (
              <div key={post.id} className='flex flex-col w-11/12 md:w-7/12 max-w-4xl bg-gray-800 p-2 mb-4 mt-4 rounded'>
                <div className='flex flex-row items-center'>
                  <Link href={{ pathname: "/profile", query: { userId: post.user_id } }}>
                ...
                <Carousel showStatus={false} showThumbs={false}>
                  {post.imageurls.map((image, index) => (
                    <div key={index} className="w-full h-4/5 flex items-center justify">
                ...
                <div className='flex flex-col '>
                  ...
                  {showComments[post.id] && post.comments.map((comment, index) => (
                    <div key={index} className='flex flex-col border-b-2'>
                      <div className='flex flex-row items-center'>
                        <Link href={{ pathname: "/profile", query: { userId: comment.commenterId } }}>
                          <div className='flex items-center'>
